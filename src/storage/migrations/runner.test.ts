...
describe('runMigrations', () => {
  ...
  it('creates _migrations table and applies all migrations', async () => {
    ...
    const applied = await db.execO('SELECT version, name FROM _migrations ORDER BY version');
    ...
    // Tables should exist
    expect(
      await db.execA("SELECT name FROM sqlite_master WHERE type='table' AND name='foo'"),
    ).toHaveLength(1);
    expect(
      await db.execA("SELECT name FROM sqlite_master WHERE type='table' AND name='bar'"),
    ).toHaveLength(1);
  });
  ...
  it('skips already-applied migrations', async () => {
    ...
    const applied = await db.execO('SELECT version FROM _migrations ORDER BY version');
    expect(applied).toHaveLength(2);
  ...
  it('throws on bad SQL with version info', async () => {
    const bad: Migration = { version: 3, name: 'bad-sql', up: 'NOT VALID SQL' };

    await expect(runMigrations(db, [bad])).rejects.toThrow(/Migration 3 \(bad-sql\) failed/);
  });
  ...
  it('supports multi-statement migrations', async () => {
    ...
    expect(
      await db.execA("SELECT name FROM sqlite_master WHERE type='table' AND name='a'"),
    ).toHaveLength(1);
    expect(
      await db.execA("SELECT name FROM sqlite_master WHERE type='table' AND name='b'"),
    ).toHaveLength(1);
  });
});
