...
import { connectToExtension, type BridgeMessage } from '../../sync/investpass-bridge.ts';
import { ImportPayloadSchema } from '../../workers/investpass-types.ts';
import type {
  InvestPassImportResult,
  InvestPassTransaction,
} from '../../workers/investpass-types.ts';
...
export function useInvestPassImport(extensionId: string): UseInvestPassImportReturn {
  ...
  const runImport = useCallback(
    async (transactions: InvestPassTransaction[]) => {
      setStatus('importing');
  ...
  const mapAccount = useCallback(
    async (investPassName: string, mytruetrackAccountId: string) => {
      await saveMapping({
